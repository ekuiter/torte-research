...
- We deliberately omit some systems:
  ...
  - We exclude several systems that we consider system software, but which do not explicitly model features and their dependencies using a dedicated language.
    ...
    For example, we exclude pure runtime variability, compile-time options via `.h` header files, the code generator Autoconf/M4 (aka `./configure`), custom `Makefile` patterns, or obscure build systems (e.g.,  BitBake recipes).
    ...
    `dietlibc`, `glibc`, `musl`, `Yocto`/`OpenEmbedded`
